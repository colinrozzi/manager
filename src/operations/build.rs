...
pub fn handle_build_command(app_state: &mut AppState, frontend_channel_id: &str) -> Result<(), String> {
    ...
    let runtime_info = match runtime_info_value.get("data") {
        Some(data) => data,
        None => {
            let error_msg = "Missing data field in filesystem info";
            log(error_msg);
    ...
    let fs_info: InfoResult = match serde_json::from_value(runtime_info.clone()) {
        ...
        Err(e) => {
            ...
            return Err(format!("Failed to parse filesystem info data: {}", e));
        }
    };
    
    log(&format!("Filesystem info: hash={}, store_id={}", fs_info.head_hash, fs_info.store_id));
    ...
    let build_actor_id = match spawn("/Users/colinrozzi/work/actors/build-actor/actor.toml", None) {
        ...
        Err(e) => {
            ...
            return Err(format!("Failed to spawn build actor: {}", e));
        }
    };
    ...
    let build_channel_id = match open_channel(&build_actor_id, &channel_init_bytes) {
        ...
        Err(e) => {
            ...
            return Err(format!("Failed to open channel to build actor: {}", e));
        }
    };
    
    log(&format!("Opened channel to build actor: {}", build_channel_id));
    ...
    app_state.actor_channels.insert(operation_id.clone(), build_channel_id.clone());
    ...
    let build_command_bytes = match serde_json::to_vec(&build_command) {
        ...
        Err(e) => {
            ...
            return Err(format!("Failed to serialize build command: {}", e));
        }
    };
    ...
    if let Err(e) = send_on_channel(&build_channel_id, &build_command_bytes) {
        ...
        return Err(format!("Failed to send build command: {}", e));
    }
