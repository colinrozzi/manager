...
pub fn handle_change_command(
    ...
    log(&format!(
        "Processing ChangeRequest command: {}",
        description
    ));
    ...
    let programmer_channel_id =
        match open_channel(&app_state.programmer_actor_id, &channel_init_bytes) {
            Ok(id) => id,
            Err(e) => {
    ...
    log(&format!(
        "Opened channel to programmer actor: {}",
        programmer_channel_id
    ));
