use crate::bindings::ntwk::theater::message_server_host::{send_on_channel, close_channel};
...
use crate::operations::{process_frontend_command, send_status_update};
use crate::operations::utils::get_current_time;
...
pub fn handle_frontend_setup(app_state: &mut AppState, channel_id: &str) -> Result<(), String> {
    ...
    app_state.register_channel(channel_id.to_string(), crate::messaging::ChannelType::Frontend);
...
pub fn handle_frontend_message(app_state: &mut AppState, channel_id: &str, message_data: &[u8]) -> Result<(), String> {
    // Process frontend commands
    match serde_json::from_slice::<FrontendCommand>(message_data) {
        Ok(command) => {
            // Process the command
            if let Err(e) = process_frontend_command(app_state, command, channel_id) {
...
pub fn handle_build_message(app_state: &mut AppState, channel_id: &str, operation_id: &str, message_data: &[u8]) -> Result<(), String> {
    ...
    let operation = match app_state.active_operations.get_mut(operation_id) {
        Some(op) => op,
        None => {
            log(&format!("Operation {} not found for build message", operation_id));
            return Ok(());
        }
    };
...
pub fn handle_programmer_message(app_state: &mut AppState, channel_id: &str, operation_id: &str, message_data: &[u8]) -> Result<(), String> {
    // Get frontend channel
    let frontend_channel = match &app_state.frontend_channel_id {
        Some(id) => id,
        None => {
    ...
    let operation = match app_state.active_operations.get_mut(operation_id) {
        Some(op) => op,
        None => {
            log(&format!("Operation {} not found for programmer message", operation_id));
            return Ok(());
        }
    };
...
pub fn handle_unknown_message(app_state: &mut AppState, channel_id: &str, message_data: &[u8]) -> Result<(), String> {
    log(&format!("Received message from unknown channel type: {}", channel_id));
